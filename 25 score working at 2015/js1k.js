...
// 1015 bytes
for(_="textareaF</F><R=='QQ#'[2]addMOV=[))y((A[4]y+C),'#',0\\n 4ADD ?A:h;JM id=)=>')Timeout(v(m,D,E	.innerHTML=);.length,B,C=A=>h=)( +([#@<]?) *(.+?.includes(i)?`<b style='background-color:#b`<FW> 0 1RFY> 5 <5SUB #5#63 2JMP -4DAT 0 2RpreP></pre>`;v+8e9)%8e3;yc[A)];x=B?C:A+C+(B?=-(BQ<):0u=,b,J=p=0A.value.replace(/^(...?)?( *;.*)?$/gm,(m,A,a,d	J||A&&(J=AQEND?p=C:c[b++)]A|0	|0])|p;_=DAT-1;=k?+i*A:(m=l+m*A),l+*A)ADD(1SUB(-1CMPh+=k?i==:l+''==m;DJN--(D?h):m)PA;JMNZ?h:A;k?=i:c[j)]...l];SLTh+=(k?i:l)<;SPLg.push(++h&&A;(P.onclick(n=a=h{g=o%2?d:f;h=g.shift([A	]=hi=x(hj=x(h	k=B;l=im=jeval)(i-1)h>=0&&SPL(P(g?--o?(n=seta,1&&[o,d,f]:'Time':o%2?'B!':'A!+c.map(([[A]],i(i%150?'':'+(d109'>`:f900'>`:'+A+'</b>.join('})(clearn),c=Arra8e3).fill(o=8e4).map=>h'_']),fu(Y,e=100+~~(Math.random()*7800+e],du(W,0)]()";G=/[-QRF]/.exec(_);)with(_.split(G))_=join(shift());console.log(_);eval(_)
/**/
/*
